...
void BornAgainDecisionTree::exportBATree(std::string fileName)
{
	...
	if (myfile.is_open())
	{
		...
		myfile << "Format: nodeID | terminal | leftChild | rightChild | splitvarID | splitval | depth | prediction" << std::endl;
		myfile << std::endl;
		...
		for (int i = 0; i < (int)rebornTree.size(); i++)
		{
			if (rebornTree[i].nodeType == Node::NODE_INTERNAL)
				myfile << i << " FALSE " << rebornTree[i].leftChild << " " << rebornTree[i].rightChild << " " << rebornTree[i].splitFeature << " " << rebornTree[i].splitValue << " " << rebornTree[i].depth << " -1" << std::endl;
			else if (rebornTree[i].nodeType == Node::NODE_LEAF)
				myfile << i << " TRUE -1 -1 -1 -1 " << rebornTree[i].depth << " " << rebornTree[i].classification << std::endl;
